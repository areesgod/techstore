...
Диссертация состоит из введения, восьми глав, заключения, списка литературы (49 источников) и приложений. Общий объём --- 90--110 страниц.
...
UUID-токен генерируется при регистрации, отправляется через Resend API с домена \texttt{tech-store.site}. Срок действия --- 24 часа. Неверифицированные пользователи не могут оформлять заказы.

После перехода по ссылке верификации бэкенд возвращает JWT-токен вместе с подтверждением, что позволяет фронтенду выполнить автоматический вход без дополнительного шага. Пользователь перенаправляется на главную страницу через 2 секунды.
...
\begin{itemize}
  \item FR-20: Дашборд с ключевыми метриками
  \item FR-21: CRUD-управление товарами
  \item FR-22: Управление всеми заказами
  \item FR-23: Управление пользователями (назначение ролей, привязка к филиалу)
\end{itemize}

\textbf{Сотрудник (EMPLOYEE):}
\begin{itemize}
  \item FR-24: Просмотр заказов своего филиала с фильтрацией по статусу
  \item FR-25: Изменение статуса заказа (pending → processing → shipped → delivered)
\end{itemize}
...
\begin{enumerate}
  ...
  \item Все 25 функциональных требований выполнены; p95 времени ответа --- 287 мс.
